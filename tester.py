...
def main(sdk: SDK, args, general_args, hyperparameter_args):
    ...
    if len(hyperparameter_args.models) == 0:
        hyperparameter_args.models = random.choices(list(model_map.keys()), k=args.num_tasks)
    ...
    assert (
        args.num_tasks == len(hyperparameter_args.models)
    ), f"The number of tasks ({args.num_tasks}) should match the number of models ({len(hyperparameter_args.models)})"

    task_ids = set()

    for idx, (model_name, inference) in enumerate(zip(hyperparameter_args.models, inference_flags), 1):
